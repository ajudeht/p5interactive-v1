...
function draw() {
    ...
    stroke(color(0, 79, 255));
    var spectrum = fft.analyze();

    beginShape();

    for (i = 0; i < spectrum.length; i++) {
        vertex(i, map(spectrum[i], 0, windowWidth / 2, height + height/8, 0) - (windowHeight / 2));
        //vertex(i, map(spectrum[i], 0, windowWidth / 2, height, 0) - (windowHeight / 4));
    }
    endShape();
